...
function parseThrow(subset: string): Throw {
    const set: Throw = Object.fromEntries(subset.split(', ').map(pair => {
...
function isThrowPossible(set: Throw): boolean {
    return ((set?.red ?? 0) <= MAX_AVAILABLE.red && (set?.green ?? 0) <= MAX_AVAILABLE.green && (set?.blue ?? 0) <= MAX_AVAILABLE.blue)
}

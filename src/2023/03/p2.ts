...
function getGearsRatio(grid: Grid): number[] {
    ...
    grid.forEach((row) => {
        row.forEach((col) => {
            surroundingNumbers = surroundingNumbers.concat(
                filterSurroundNumbers(getSurroundNumbers(grid, col), grid)
            )
        })
    })
    ...
    for (let i = 0; i < partsIdx.length - 1; i = i + 2) {
        ...
        output.push(
            Number(part1Idx.split('-').at(2)) *
                Number(part2Idx.split('-').at(2))
        )
    }
...
function filterSurroundNumbers(surroundingNumbers: Col[], grid: Grid): Col[] {
    ...
    if (
        Math.max(...colsIdx) - Math.min(...colsIdx) > 1 &&
        !centerCol.value.match(/\d/)
    ) {
        return surroundingNumbers
    }

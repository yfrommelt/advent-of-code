...
function getSource(destination: number, almanac: Almanac): number {
    ...
    almanac.some((correspondance) => {
        if (
            destination >= correspondance.destinationStart &&
            destination <=
                correspondance.destinationStart + correspondance.range
        ) {
            source +=
                correspondance.sourceStart - correspondance.destinationStart
            return true
        }
    })
    return source
